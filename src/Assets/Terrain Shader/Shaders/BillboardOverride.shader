...
#define ALBEDO_NORMAL_LIGHTING 1
        ...
        void CameraFacingBillboardVert(inout float4 pos, float2 offset, float offsetz)
        {
            ...
            pos.xyz += up * radius;
				...
				float _BillboardFireSize;
				float _BillboardFirePosition;
				fixed3 _BillboardBurnColor;
                ...
                fixed4 frag(v2f input) : SV_Target
                {
					...
					//add burn color
					//TODO find a way to override this with another generated prefab texture
					BurnState simState = currentBurnState(_BillboardFireSize, _BillboardFirePosition, input.worldPos);
					col.rgb = lerp(fixed3(0.2, 0.2, 0.2) * col.rgb, col.rgb, simState.burnState);
					col.rgb += _BillboardBurnColor * simState.fireLine * 2.0;
...
#endif
					//TODO find a way to override this with another generated prefab texture
					//make disappear based on burn
					col.a *= lerp(0.1, 1.0, simState.burnState);
