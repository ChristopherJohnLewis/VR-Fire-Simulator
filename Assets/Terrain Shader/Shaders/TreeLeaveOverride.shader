...
Shader "Custom/Tree Leaves Override" {
Properties {
    _Color ("Main Color", Color) = (1,1,1,1)
	_EmissionColor ("Burn Color", Color) = (1,1,1,1)
...
void surf (Input IN, inout LeafSurfaceOutput o) {
	...
	o.Emission = _EmissionColor * simState.fireLine *2.0f;

	//modify tree texture to burn
	o.Albedo = lerp(tex2D(_MainBurnedTex, IN.uv_MainTex), o.Albedo, simState.burnState);
	o.Alpha = lerp(tex2D(_MainBurnedTex, IN.uv_MainTex).a, c.a, simState.burnState);
...
ENDCG
    ...
    Pass {
        ...
        float4 frag_surf (v2f_surf IN) : SV_Target {
            ...
            half alpha = lerp(tex2D(_MainBurnedTex, IN.hip_pack0.xy).a, tex2D(_MainTex, IN.hip_pack0.xy).a, simState.burnState);
            clip (alpha - _Cutoff);
...
//Dependency "BillboardShader" = "Custom/Tree Leaves Rendertex Override"
}
